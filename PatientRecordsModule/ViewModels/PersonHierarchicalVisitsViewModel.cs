...
namespace PatientRecordsModule.ViewModels
{
    public class PersonHierarchicalVisitsViewModel : BindableBase
    {
        ...
        #region Constructors
        public PersonHierarchicalVisitsViewModel(VisitDTO visitDTO, IPatientRecordsService patientRecordsService, ILog logService)
        {
            if (visitDTO == null)
            {
        ...
        private async void LoadItemsAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                CriticalFailureMediator.Activate("Не удалость загрузить вложенные элементы случая. Попробуйте еще раз или обратитесь в службу поддержки", reloadPatientVisitsCommandWrapper, ex);
                loadingIsCompleted = true;
            }
            finally
            {
